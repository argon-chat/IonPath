...
public sealed class GoCodeGenerator : CodeGeneratorBase
{
        ...
        bool includeClient)
    {
        ...
        // Header and imports
        sb.AppendLine(FileHeader(CollectPackageDoc(context)));
        sb.AppendLine();
    ...
    private string GenerateServiceClient(IonService service)
    {
        ...
        // The client struct mirrors the service, so it inherits the service documentation.
        // Fall back to the previous placeholder banner when the service is undocumented.
        var serviceDoc = Emitter.DocComment(service.Doc, identifier: $"{serviceName}Client");
        if (serviceDoc.Length == 0)
            serviceDoc = $"// {serviceName}Client is a client for I{serviceName}.\n";

        var ctx = new TemplateContext()
            .Set("serviceName", serviceName)
            .Set("serviceDoc", serviceDoc)
    ...
    private string GenerateClientMethod(IonMethod method, string serviceName)
    {
        ...
        // Go client parameters keep the camelCase local-variable names from the signature.
        var docParams = method.arguments
            .Where(a => a.mod != IonArgumentModifiers.Stream)
            .Select(a => new DocParam(FormatLocalVariableName(a.name.Identifier), a.Doc))
            .ToList();
        ...
        var ctx = new TemplateContext()
            ...
            .Set("methodDoc", Emitter.DocComment(method.Doc, parameters: docParams, identifier: methodName))
    ...
    public override string FileHeader() => FileHeader(null);

    /// <summary>
    /// File header plus the optional package doc comment, which by Go convention sits
    /// immediately above the <c>package</c> clause.
    /// </summary>
    private string FileHeader(string? packageDoc) => $"""
        // Code generated by ionc. DO NOT EDIT.
        // source: ion protocol definition

        {packageDoc}package {_packageName}
        """;

    /// <summary>
    /// Joins the <c>//!</c>-style module documentation of every compiled module into a
    /// single Go package doc comment. Returns <c>null</c> when nothing is documented.
    /// </summary>
    private string? CollectPackageDoc(CompilationContext context)
    {
        var merged = CollectModuleDocs(context);
        if (merged is null)
            return null;

        var comment = Emitter.ModuleDocComment(merged, _packageName);
